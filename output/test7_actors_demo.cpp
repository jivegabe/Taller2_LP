...
// Actor: alertas
class alertasActor : public Actor {
public:
    alertasActor() {
        set_handler([this](any _msg) { this->handle(_msg); });
    }
...
// Actor: notifier
class notifierActor : public Actor {
public:
    notifierActor() {
        set_handler([this](any _msg) { this->handle(_msg); });
    }
...
int main() {
    [&]() { auto log = actorSystem.spawn<loggerActor>(); auto alert = actorSystem.spawn<alertasActor>(); auto stat = actorSystem.spawn<statsActor>(); auto notify = actorSystem.spawn<notifierActor>(); log->send("[LOG] Sistema iniciado"); log->send("[LOG] Conexion establecida"); alert->send("[ALERTA] Usuario admin conectado"); stat->send("[STAT] CPU: 45%"); stat->send("[STAT] RAM: 2.3GB"); notify->send("[NOTIFY] Nuevo mensaje recibido"); log->send("[LOG] Procesando solicitud #1001"); log->send("[LOG] Procesando solicitud #1002"); alert->send("[ALERTA] Intento de acceso fallido"); stat->send("[STAT] Solicitudes/seg: 150"); notify->send("[NOTIFY] Tarea completada"); log->send("[LOG] Backup iniciado"); alert->send("[ALERTA] Disco al 85%"); stat->send("[STAT] Disco: 85%"); notify->send("[NOTIFY] Backup programado"); log->send("[LOG] Sistema estable"); alert->send("[ALERTA] Todo normal"); stat->send("[STAT] Uptime: 24h"); notify->send("[NOTIFY] Reporte generado"); println("=== Main: Todos los mensajes enviados ==="); return 0; }();
